...
var parse = function (tokens) {
	...
	var expect = function (type) {
		//console.log('expect', String(cursor()), cursor().type);
		var res = cursor();
		if (!res.type === type) {
	...
	var makeNull = function () {
		...
		/*if (String(token) !== 'null') {
			throw new Error('null was not null: ' + String(token));
		}*/
		return null;
	};
	...
	var makeTrue = function () {
		...
		/*if (String(token) !== 'true') {
			throw new Error('true was not true: ' + String(token));
		}*/
		return true;
	};
	...
	var makeFalse = function () {
		...
		/*if (String(token) !== 'false') {
			throw new Error('false was not false: ' + String(token));
		}*/
		return false;
	};
	...
	var makeObject = function () {
		//console.log('makeObject', String(cursor()), cursor().type);
	...
	var makeArray = function () {
		//console.log('makeArray', String(cursor()), cursor().type);
		var res = [];
		expect(ARRAY_BEGIN);
	...
	var makeValue = function () {
		//console.log('makeValue', String(cursor()), cursor().type);
		switch (cursor().type) {
			case NULL:
				return makeNull();
			case TRUE:
...
var pp = function (json) {
	tokenize(json).forEach(function (token) {
		console.log(String(token), token.type);
	});
};
